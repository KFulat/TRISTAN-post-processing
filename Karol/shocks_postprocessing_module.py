...
movHR_path = Path('../result/movHR')
phase_path = Path('../result/phase')
distr_path = Path('../result/distr')
dens_plot_path = '../movie_dens_scripts/'
bfield_plot_path = '../movie_bfield_scripts/'
phase_plot_path = './'
...
def load_phase_data(pxe, pye, pze, step):
    ...
    file_id = h5py.File(phase_path/file_name)
...
def plot_dens(dense, densi, xp, yp, x_shock_linear, x_shock, step):
    ...
    fig = plt.figure(figsize=(12.31, 11.1), dpi=300)
    ...
    cbaxes = fig.add_axes([0.95, 0.5095, 0.015, 0.3515])
    ...
    ax1.axvline(x_shock, color='white', linestyle='dashed', linewidth=2)
    ax2.axvline(x_shock, color='white', linestyle='dashed', linewidth=2)
    ...
    ax1.text(xlim[0]+79.5, ylim[1]+2, 'nstep='+step_str, fontsize=fs_text)
    ax1.text(xlim[0]+34, ylim[1]+2, r'$t=$'+time_str+' '+r'$\Omega_{i}^{-1}$',
             fontsize=fs_text)
    ax1.text(xlim[0]+50, ylim[1]+2, '(log, floor='+floor_str+')',
             fontsize=fs_text)
    ax1.text(xlim[0], ylim[1]+1, 'electron density', fontsize=fs_text)
    ax2.text(xlim[0], ylim[1]+1, 'ion density', fontsize=fs_text)
    ...
    cbar.set_label(r'$N/N_0$', fontsize=fs_labels)
...
def plot_bfield(bx, by, bz, xp, yp, x_shock_linear, x_shock, step):
    ...
    # Contour plots
    levels = 21
    clvls = np.linspace(-2.8, 2.8, levels)
    cset1 = ax1.contourf(xp, yp, bx.T, clvls, cmap='jet')
    cset2 = ax2.contourf(xp, yp, bz.T, clvls, cmap='jet')
    ...
    # Axes labels
    ax1.set_ylabel(r'$y/\lambda_{si}$ $B_x$', fontsize=fs_labels)
    ax2.set_xlabel(r'$x/\lambda_{si}$', fontsize=fs_labels)
    ax2.set_ylabel(r'$y/\lambda_{si}$ $B_z$', fontsize=fs_labels)

    # Text
    # ax1.text(xlim[1]-12,ylim[1]-4.8,r'$a) N_e/N_0$', fontsize=fs_text)
    # ax2.text(xlim[1]-12,ylim[1]-4.8,r'$b) N_i/N_0$', fontsize=fs_text)
    ax1.text(xlim[0]+79.5, ylim[1]+2, 'nstep='+step_str, fontsize=fs_text)
    ax1.text(xlim[0]+34, ylim[1]+2, '{}{} {}'.format(r'$t=$', time_str, r'$\Omega_{i}^{-1}$'),
             fontsize=fs_text)
    # ax1.text(xlim[0]+34, ylim[1]+2, r'$t=$'+time_str+' '+r'$\Omega_{i}^{-1}$',
    #          fontsize=fs_text)
    ax1.text(xlim[0]+50, ylim[1]+2, '(log, floor='+floor_str+')',
    ...
    # Colorbar
    cticks = np.linspace(-2.8, 2.8, 11)
    cbar = fig.colorbar(cset1, cax=cbaxes, ax=[ax1, ax2], ticks=cticks)
    cbar.ax.tick_params(labelsize=fs_clabel, length=5)
    cbar.ax.set_yticklabels(["{:3.1f}".format(i) for i in cticks])
    cbar.set_label(r'$B/B_0$', fontsize=fs_labels)

    return fig


def plot_bfield3(bx, by, bz, xp, yp, x_shock_linear, x_shock, step):
    time_str = "{:04.1f}".format(step/GAMMA)
    step_str = "{:07d}".format(step)
    floor_str = "{:02.1f}".format(AFLOORB)

    fig = plt.figure(figsize=(12.0, 4.815), dpi=300)
    fs = [12, 16, 20]
    levels = 21
    clvls = np.linspace(-2.8, 2.8, levels)
    ...
    cbaxes = fig.add_axes([0.95, 0.068, 0.01, 0.885])

    # BX
    cset1 = ax1.contourf(xp, yp, bx.T, clvls, cmap='jet')
    ax1.axvline(x_shock, color='white', linestyle='dashed', linewidth=1)
    ax1.set_aspect('equal')
    ax1.set_ylabel(r'$y/\lambda_{si}$ $B_x$', fontsize=fs[0])
    plt.setp(ax1.get_xticklabels(), visible=False)
    ax1.yaxis.set_major_locator(MultipleLocator(4))
    ax1.yaxis.set_minor_locator(MultipleLocator(1))
    ...
    ax2.set_aspect('equal')
    ax2.set_ylabel(r'$y/\lambda_{si}$ $B_y$', fontsize=fs[0])
    ...
    ax3.set_xlabel(r'$x/\lambda_{si}$', fontsize=fs[0])
    ax3.set_ylabel(r'$y/\lambda_{si}$ $B_z$', fontsize=fs[0])
    ...
    cbar = fig.colorbar(cset1, cax=cbaxes, ax=[ax1, ax2, ax3], ticks=clvls)
    cbar.set_label(r'$B/B_0$', fontsize=fs[1])
    cbar.ax.set_yticklabels(["{:4.2f}".format(i) for i in clvls])
...
def plot_phase(px, py, pz, xp, yp, x_shock_linear, x_shock, step):
    ...
    fig = plt.figure(figsize=(12.125, 7.0), dpi=300)
    fs = [12, 16, 20]
    levels = 37
    clvls = np.linspace(0, 3.6, levels)
    cbar_ticks = np.linspace(0, 3.6, 10)
    ...
    ax2 = fig.add_axes([0.05, 0.36, 0.88, 0.3], sharex=ax3, ylim=ylim)
    ax1 = fig.add_axes([0.05, 0.67, 0.88, 0.3], sharex=ax3, ylim=ylim)
    cbaxes = fig.add_axes([0.95, 0.67, 0.01, 0.3])

    # PX
    cset1 = ax1.contourf(xp, yp, px.T, clvls, cmap='nipy_spectral')
    ax1.axvline(x_shock, color='black', linestyle='dashed', linewidth=1)
    ax1.set_ylabel(r'$p_{xe}/m_e$', fontsize=fs[0])
    plt.setp(ax1.get_xticklabels(), visible=False)
    ...
    # PY
    cset2 = ax2.contourf(xp, yp, py.T, clvls, cmap='nipy_spectral')
    ax2.axvline(x_shock, color='black', linestyle='dashed', linewidth=1)
    ax2.set_ylabel(r'$p_{ye}/m_e$', fontsize=fs[0])
    plt.setp(ax2.get_xticklabels(), visible=False)
    ...
    # PZ
    cset3 = ax3.contourf(xp, yp, pz.T, clvls, cmap='nipy_spectral')
    ax3.axvline(x_shock, color='black', linestyle='dashed', linewidth=1)
    ax3.set_xlabel(r'$x/\lambda_{si}$', fontsize=fs[0])
    ax3.set_ylabel(r'$p_{ze}/m_e$', fontsize=fs[0])
    ...
    cbar = fig.colorbar(cset1, cax=cbaxes, ax=[ax1, ax2, ax3],
                        ticks=cbar_ticks)
...
def plot_spectra_downstream(
    ...
    for layer in range(layers_number):
        ...
        # ax = fig.add_axes([0.12, 0.11, 0.8, 0.84], xlim=xlim, ylim=ylim)
        ax = fig.add_axes([0.12, 0.11, 0.8, 0.84], xlim=xlim, ylim=ylim)
        # cbaxes = fig.add_axes([0.93, 0.11, 0.03, 0.796])
        cbaxes = fig.add_axes([0.85, 0.35, 0.04, 0.52])
        ...
        ax.set_ylabel(r'$\mathrm{(\gamma-1)(dN/d\gamma)/N}$', fontsize=fs_labels)
        ax.set_xlabel(r'$\mathrm{\gamma-1}$', fontsize=fs_labels)
        ax.tick_params(axis='both', pad=8, labelsize=fs_ticks, length=10)
        ax.tick_params(axis='both', which='minor', length=5)
        # ax.text(xlim[0]+3e-3, ylim[0]+5e-6, 'downstream', fontsize=fs_text)
        ax.text(xlim[0]+6e-3, ylim[0]+2e-6, 'downstream', fontsize=fs_text)
        ax.text(xlim[0]+6e-3, ylim[0]+9e-6, r'$\theta_{Bn}=\mathrm{75^o}$', fontsize=fs_text)
        ax.text(xlim[0]+6e-3, ylim[0]+35e-6, r'$\beta=20$', fontsize=fs_text)

        # x_range_string_r = x_left_downstream+layer*layers_distance/LSI
        # x_range_string_l = (x_range_string_r-layers_thikness/LSI)
        # ax.text(xlim[0]+3e-3, ylim[0]+1e-6, r'$x_{sh}-x=(%d-%d)\lambda_{si}$'
        #         % (x_range_string_l, x_range_string_r), fontsize=fs_text)

        cbar = fig.colorbar(cmap, cax=cbaxes, ticks=ticks)
        cbar.ax.set_title(r'$\mathrm{\Omega_i t}$', fontsize=fs_clabel)
        cbar.ax.tick_params(labelsize=(fs_clabel-2), length=5)
        cbaxes.yaxis.set_ticks_position('left')

        # Maxwellian fit
        maxwellian_curve = xp*maxwell_fit(0.03, 1.0, xp, spectra[layer][-1])
        ax.plot(xp, maxwellian_curve, linestyle='--', color='black')

        # Power fit
        power_index = np.int(pltnumber-1)
        power_curve, p = power_fit(0.2, 2.0, xp, spectra[layer][power_index])
        ax.plot(xp[290:], power_curve[290:], linestyle='--', color='black')
        ax.text(5, 1e-3, r'$\mathrm{p=}$'+"{:.1f}".format(p), fontsize=fs_text)
        # ax.plot(xp[290:], 0.01*xp[290:]**(-2.4), linestyle='--', color='black')
        # ax.text(10, 1e-3, r'$p=$'+"{:.2f}".format(-2.4), fontsize=fs_text)
...
def plot_spectra_upstream(
    ...
    for layer in range(layers_number):
        fig = plt.figure(figsize=(8.5, 6), dpi=300)
        ax = fig.add_axes([0.12, 0.11, 0.8, 0.84], xlim=xlim, ylim=ylim)
        # ax_gamma = fig.add_axes([0.19, 0.31, 0.29, 0.29], ylim=(5, 25))
        # cbaxes = fig.add_axes([0.93, 0.11, 0.03, 0.796])
        cbaxes = fig.add_axes([0.85, 0.35, 0.04, 0.52])
        ...
        ax.tick_params(axis='both', pad=8, labelsize=fs_ticks, length=10)
        ax.tick_params(axis='both', which='minor', length=5)
        # ax.text(xlim[0]+3e-3, ylim[0]+5e-6, 'upstream', fontsize=fs_text)
        ax.text(xlim[0]+6e-3, ylim[0]+2e-6, 'upstream', fontsize=fs_text)
        ax.text(xlim[0]+6e-3, ylim[0]+9e-6, r'$\theta_{Bn}=\mathrm{75^o}$', fontsize=fs_text)
        ax.text(xlim[0]+6e-3, ylim[0]+35e-6, r'$\beta=20$', fontsize=fs_text)
        ...
        # ax.text(xlim[0]+3e-3, ylim[0]+1e-6, r'$x-x_{sh}=(%d-%d)\lambda_{si}$' %
        #         (x_range_string_l, x_range_string_r), fontsize=fs_text)
        cbar = fig.colorbar(cmap, cax=cbaxes, ticks=ticks)
        cbar.ax.set_title(r'$\mathrm{\Omega_i t}$', fontsize=fs_clabel)
        cbar.ax.tick_params(labelsize=(fs_clabel-2), length=5)
        ...
        # # Maxwellian fit
        SDA = np.loadtxt("SDA75.txt")
        # maxwellian_curve = xp*maxwell_fit(0.03, 1.0, xp, spectra[layer][0])
        maxwellian_curve = xp*maxwell_fit(0.03, 1.0, xp, SDA)
        ax.plot(xp, maxwellian_curve, linestyle='--', color='black')

        # Power fit
        # ax_gamma.plot(time, gamma_max[layer, :, 0], label="cutoff 1e-4")
        # ax_gamma.plot(time, gamma_max[layer, :, 1], label="cutoff 1e-5")
        # ax_gamma.legend(loc='upper left', fontsize=11)
        # ax_gamma.set_xlabel(r'$t \/ \Omega_i$', labelpad=2, fontsize=11)
        # ax_gamma.set_ylabel(r'$\gamma_{max}$', labelpad=2, fontsize=11)
        # ax_gamma.tick_params(axis='both', pad=2, labelsize=11, length=5)

        # Theoretical SDA curve
        ax.loglog(xp, SDA, "-", color='magenta', linewidth=2.0)
