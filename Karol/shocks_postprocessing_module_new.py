...
def load_density_data(dense, densi, step):
    ...
    # Group opening
    group_id = file_id["Step#"+group_str]
...
def plot_dens(dense, densi, xp, yp, x_shock_linear, x_shock, step):
    ...
    fig = plt.figure(figsize=(12.31*0.6, 9.2*1.118), dpi=300)
    ...
    cbaxes = fig.add_axes([0.95, 0.4955, 0.015, 0.379])
...
def plot_bfield(bx, by, bz, xp, yp, x_shock_linear, x_shock, step):
    ...
    fig = plt.figure(figsize=(12.31*0.6, 12.3*1.118), dpi=300)
...
def plot_phase(px, py, pz, xp, yp, x_shock_linear, x_shock, step):
    ...
    # Contour plots
    levels = 41
    clvls = np.linspace(0, 4.0, levels)
